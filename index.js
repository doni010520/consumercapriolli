...
pool.connect((err, client, release) => {
  if (err) console.error('❌ Erro ao conectar ao Supabase:', err.stack);
  else {
...
/* ========================= Helpers ========================= */
const sendError = (res, http, msg, err) =>
...
/* ========================= Auth ========================= */
// Compatível com o que você tinha ANTES (Authorization: Bearer <TOKEN>),
// mas também aceita variações que algumas instalações do Consumer usam.
function authenticate(req, res, next) {
  ...
  // 1) Igual o comportamento antigo: "Authorization: Bearer <TOKEN>"
  const bearerMatch = auth.match(/^Bearer\s+(.+)$/i);
  if (bearerMatch) provided = bearerMatch[1];

  // 2) Se veio "Authorization: <TOKEN>" sem 'Bearer'
...
app.get('/api/order/:orderId', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query('SELECT dados, status FROM pedidos WHERE id = $1', [orderId]);
    if (rows.length === 0) return sendError(res, 404, 'Pedido não encontrado');
...
app.post('/api/order/details', async (req, res) => {
  try {
    ...
    if (!orderId) return sendError(res, 400, 'ID do pedido é obrigatório');
...
app.post('/api/order/status', async (req, res) => {
  try {
    ...
    if (!orderId || !status) return sendError(res, 400, 'orderId e status são obrigatórios');

    const { rowCount } = await pool.query('SELECT 1 FROM pedidos WHERE id = $1', [orderId]);
    if (rowCount === 0) return sendError(res, 404, 'Pedido não encontrado');

    await pool.query(`UPDATE pedidos SET status = $1, updated_at = NOW() WHERE id = $2`, [status, orderId]);
    await pool.query(
  ...
  try {
    const pedidoTeste = {
      ...
      delivery: {
        ...
        deliveryAddress: { streetName: 'Rua Teste', streetNumber: '123', neighborhood: 'Bairro Teste', city: 'São Paulo', state: 'SP', postalCode: '01234-567', country: 'BR' }
      }
    };
